...
We need to do some configuration of the package before we can build it. LLVM and its binary packages are versioned, but a few of the shared libraries it installs are not. The so-called "common packages", which you can see listed in `debian/packages.common`, have stable ABIs according to upstream. The Debian maintainers have developed a configuration for this in the form of the `SKIP_COMMON_PACKAGES` and `NEW_LLVM_VERSION` environment variables which are set in `debian/rules`. They are designed for a scenario where the most recent version of LLVM is used to build the unversioned packages, and then all other version in the archive are sure to be older, and so link against the newer package runtimes. When `SKIP_COMMON_PACKAGES` is set to `no`, it will build the runtimes in their unversioned form. When it's set to `yes`, it will skip building those and instead link against the ones generated by `NEW_LLVM_VERSION`. So if version 21 is the default, and Debian is shipping a fix for version 20, they would need to set `SKIP_COMMON_PACKAGES` to `yes` and `NEW_LLVM_VERSION` to `21`, which will inject the versions expected from 21 into the build dependencies.
